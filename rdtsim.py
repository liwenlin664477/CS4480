...
class EntityA:
    ...
    def __init__(self, seqnum_limit):
        ...
        self.timeIncrement = 10 + 4 * self.windowSize
        self.progress = False
    ...
    def output(self, message):
        if len(self.m) < self.windowSize and len(self.extra) == 0:
            self.currentMessage = message
            self.checksum = self.create_checksum(self.seqnum, message.data)
            packet = Pkt(self.seqnum, self.acknum, self.checksum, message.data)
            self.currentPacket = packet
            self.m.append(packet)
            to_layer3(self, packet)
            if not self.timerIsRunning:
                self.timerIsRunning = True
                start_timer(self, self.timeIncrement)
            self.seqnum += 1
            if self.seqnum == self.seqnum_limit:
                self.seqnum = 0
        else:
    ...
    def input(self, packet):
        Checksum = self.create_checksum(packet.acknum, packet.payload)
        ...
        if Checksum == packet.checksum:
            ...
            if len(self.extra) > 0 and (len(self.m) < self.windowSize):
                while len(self.extra) > 0 and (len(self.m) < self.windowSize):
                    message = self.extra.pop(0)
                    if len(self.m) < self.windowSize:
                        self.currentMessage = message
                        self.checksum = self.create_checksum(self.seqnum, message.data)
                        packet = Pkt(self.seqnum, self.acknum, self.checksum, message.data)
                        self.currentPacket = packet
    ...
    def timer_interrupt(self):
        ...
        self.timeIncrement = self.number_timeout * self.timeIncrement
        start_timer(self, self.timeIncrement)
        self.timerIsRunning = True
        for p in self.m:
            to_layer3(self, p)
    ...
    @staticmethod
    def create_checksum(seq, payload):
        checksum = seq
        for byte in payload:
            checksum += byte
...
class EntityB:
    ...
    def __init__(self, seqnum_limit):
        ...
        self.acknum = 0
        self.number = 0
        self.lastPacket = Pkt(0, self.acknum, 0, None)

    # Called from layer 3, when a packet arrives for layer 4 at EntityB.
    # The argument `packet` is a Pkt containing the newly arrived packet.
    def input(self, packet):
        Checksum = self.create_checksum(packet.seqnum, packet.payload)
        if Checksum == packet.checksum and packet.seqnum == self.acknum:
            message = Msg(packet.payload)
            to_layer5(self, message)
            Packet = Pkt(0, self.acknum, packet.checksum, packet.payload)
            to_layer3(self, Packet)
            self.lastPacket = packet
            self.acknum += 1
            if self.acknum == self.seqnum_limit:
                self.acknum = 0
        else:
            # ack = self.acknum - 1
            # if ack == -1:
            #     ack = self.seqnum_limit - 1
            # Packet = Pkt(ack, ack, packet.checksum, packet.payload)
            to_layer3(self, self.lastPacket)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-z', type=int, default=8,
                        ...
                        help=('seqnum limit for data transport protocol; '
                              ...
                              ' [int, default: %(default)s]'))
    parser.add_argument('-l', type=float, default=0.00,
                        dest='loss_prob',
                        help=('packet loss probability'
                              ' [float, default: %(default)s]'))
    parser.add_argument('-c', type=float, default=0.00,
                        dest='corrupt_prob',
